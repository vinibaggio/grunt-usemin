...
//
// Iterate over steps. The callback is called with the following parameters:
//  - step : the step by itself
//  - last : boolean indicating if this is the last writer of the pipe
//
ConfigWriter.prototype.forEachStep = function(cb) {
  var self = this;
  this.steps.forEach( function(step, index) {
    cb(step, index === self.writersCount-1);
  });
};
...
ConfigWriter.prototype.process = function(file, config) {
  ...
  lfile.blocks.forEach(function(block) {
    ...
    self.forEachStep( function(writer, last) {
      ...
      config[writer.name] = _.extend(config[writer.name], writer.createConfig(context, block));
